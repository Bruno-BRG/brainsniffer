...
**Auditoria bibliográfica local:** preservadas as 26 chaves existentes. Conferidos DOI, autoria, título, ano e veículo presentes contra o registro histórico local, sem declarar validação remota exaustiva. Connor (`openbis`), Nsugbe (`nsugbe2022`), Kothe, TRIPOD+AI e PROBAST+AI já estavam corrigidos. Em `clinicaldata`, o DOI que já constava da nota foi também estruturado no campo `doi`; `dosei` já estava incluída. As ausências e a bibliografia manual citadas na revisão antiga estão marcadas como históricas/superadas, sem apagar a evidência. Autoria abreviada com `others`, datas de acesso, versões e termos de provedores ainda devem ser conferidos antes da submissão; não foram completados por suposição.
