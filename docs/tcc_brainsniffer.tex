...
Na anestesia geral, saber se o paciente está suficientemente hipnotizado sem overdose depende de referências objetivas de profundidade. O Índice Bispectral (Bispectral Index, BIS), calculado a partir da eletroencefalografia (EEG) frontal, é a referência mais usada à beira do leito, mas é uma saída proprietária: sua regra de combinação não é publicada e não pode ser reproduzida exatamente a partir do sinal. Modelos que tentam aproximar esse rótulo enfrentam ainda um risco metodológico silencioso: janelas vizinhas da mesma cirurgia são quase idênticas, de modo que partições aleatórias entre treino e teste medem o reconhecimento de casos já vistos, não a generalização. Falta um protótipo reproduzível que estime o rótulo BIS com partições por caso ou pessoa, controle espectral e avaliação entre fontes distintas de dados. Este trabalho apresenta e avalia o BrainSniffer, protótipo para arquivos e repetição retrospectiva de gravações (replay), com esse desenho.
...
Como controle, o baseline espectral resume cada janela com medidas clássicas interpretáveis e ajusta uma floresta aleatória (\textit{Random Forest}) nas mesmas janelas e na mesma partição do ativo, sem acesso a informação adicional. A ideia é simples: se descritores espectrais com poucos parâmetros já alcançam erro parecido, o ganho da CNN precisa ser justificado; se a CNN supera o controle com folga, há indício de que a representação aprendida captura estrutura além do espectro clássico.

Cada janela de 5~s gera 14 atributos: potências absoluta e relativa em delta (0,5--4 Hz), teta (4--8 Hz), alfa (8--13 Hz), beta (13--30 Hz) e gama (30--45 Hz), frequência de borda espectral em 90\% (SEF90), entropia espectral, valor eficaz (RMS) e \textit{line length}. Em leitura fisiológica aproximada, o aprofundamento anestésico tende a deslocar potência para bandas baixas e a reduzir SEF90 e entropia; RMS resume a amplitude e \textit{line length} combina amplitude e irregularidade, sendo sensível a transientes. Todos são calculados na mesma grade de 128~Hz, sem usar o futuro além do pré-processamento já descrito.

A floresta usa \texttt{n\_estimators}=100, \texttt{max\_depth}=12, \texttt{min\_samples\_leaf}=2 e \texttt{random\_state}=42, treinada nas 15.894 janelas de treino e avaliada nas mesmas 5.523 janelas de teste do holdout Figshare de cinco casos. Por usar a partição agrupada por caso, o controle herda a mesma proteção contra vazamento da CNN. Configuração, atributos e métricas seguem o protocolo do estudo.
...
O BrainSniffer entrega um protótipo aberto e reproduzível para estimar uma referência BIS a partir de EEG frontal em arquivos e replay retrospectivo. Como sintetizado na Seção~\ref{sec:resultados}, o checkpoint ativo mostrou desempenho interno melhor que o baseline espectral, mas perdeu associação no VitalDB; o candidato misto reduziu o erro após exposição a esse domínio, com heterogeneidade por caso e sem independência confirmatória. Os resultados são retrospectivos, exploratórios e limitados pelo rótulo, pelo número de casos, pelo reuso dos benchmarks e pelas políticas de imputação offline.
