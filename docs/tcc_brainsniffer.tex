...
\FloatBarrier
\subsection{Painéis de apoio do console interativo}

A Figura~\ref{fig:support} reúne as quatro visões de apoio do console, para que a demonstração e o texto mostrem a mesma informação. As três primeiras vêm de \artefato{reports/corpus\_manifest.json} e \artefato{models/brainsniffer\_cnn.json}; a quarta é projeção declarada, ancorada no MAE 7,03, e não curva medida \cite{brainsnifferArtefatos}.

\begin{figure}[tbp]
\centering
\includegraphics[width=\columnwidth]{figures/support_panels.pdf}
\caption{Painéis de apoio replicados no console. (a) Casos por fonte, com elegíveis, quarentena e benchmark histórico. (b) Finitude do EEG contra janelas aceitas; a linha tracejada marca o gate de 90\%. (c) Histórico de treino e validação. (d) Projeção de MAE por casos, ancorada no MAE 7,03 medido: cenário de planejamento, não curva medida \cite{brainsnifferArtefatos}.}
\label{fig:support}
\end{figure}
