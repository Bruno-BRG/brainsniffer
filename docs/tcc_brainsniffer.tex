...
O BIS é uma saída proprietária derivada de EEG processado e indicada como auxílio, em contexto e sob supervisão definidos, não como verdade universal sobre consciência, adequação anestésica ou dose. Sua leitura pode variar com eletromiografia, bloqueio neuromuscular, fármacos e atraso de processamento; em particular, estudos controlados documentaram queda do índice após bloqueio em voluntários acordados, resposta paradoxal à cetamina e atrasos distintos entre monitores processados \cite{fda2024,openbis,schuller2015,hans2005,pilge2006}. Ensaios de protocolos guiados por BIS também produziram resultados dependentes do comparador e da população, o que impede extrapolar um único número para benefício clínico universal \cite{baware,bunaware,bagrecall}.

Modelos de sinais podem aproximar índices derivados de EEG, e redes profundas já foram estudadas com alvos, canais, populações e equipamentos distintos \cite{park2020,shi2023}. Entretanto, janelas adjacentes da mesma cirurgia são dependentes: distribuí-las aleatoriamente entre treino e teste cria uma tarefa mais fácil que o uso em um novo caso. Resultados recentes no corpus Figshare e evidência metodológica mais ampla mostram que a unidade de separação altera materialmente a estimativa de desempenho \cite{sukriti2026,saeb2017}.

A pergunta de pesquisa é: quanto uma CNN compacta aproxima o rótulo BIS em casos não usados no ajuste, comparada a um baseline espectral, e como esse desempenho muda entre fontes? Este trabalho apresenta e avalia o BrainSniffer como protótipo reproduzível para arquivos e replay retrospectivo. A contribuição é uma cadeia auditável, com partições por grupo, baseline espectral, comparação entre fontes e análise de incerteza por caso. O escopo deste TCC é avaliar essa cadeia, não desenvolver um dispositivo médico \cite{brainsnifferArtefatos}.
...
\caption{Fluxo retrospectivo auditável do BrainSniffer, a partir de arquivos existentes. A separação por grupo impede que janelas do mesmo grupo atravessem partições; nos dados Figshare, o grupo é o caso e não há identificador que permita excluir repetição da mesma pessoa. O pool elegível de 33 casos não é o corpus fixo de 28 grupos: este exclui os cinco casos Figshare históricos e se divide em 16/6/6. As setas finais indicam avaliação, nunca treino com os holdouts. Fontes: manifesto do corpus e JSON dos checkpoints ativo e misto fixo. O benchmark VitalDB do misto é da mesma fonte já vista, não confirmatório \cite{brainsnifferArtefatos}.}
\label{fig:pipeline}
\end{figure}

\section{Fundamentação e trabalhos relacionados}

O conjunto ``EEG and BIS raw data'' reúne 24 casos anônimos com EEG frontal e valores BIS, publicado no Figshare sob CC BY 4.0 (Ma, 2017, v1) \cite{ma2017}. Nsugbe e Connelly estudaram o mesmo problema com características multiescala (\textit{multiscale features}) e LDA; esse trabalho sustenta a descrição do corpus e das faixas, mas não é evidência de CNN \cite{nsugbe2022}. Park et al. estimaram EEGMAC com uma rede e cadeia de aquisição, enquanto Shi et al. estimaram PSI com quatro canais em 18 participantes idosos sedados. Esses estudos demonstram viabilidade técnica em configurações específicas, não comparabilidade direta com o rótulo, o canal ou a população do BrainSniffer \cite{park2020,shi2023}.

O VitalDB contém sinais perioperatórios de milhares de cirurgias, incluindo os tracks \texttt{BIS/EEG1\_WAV} e \texttt{BIS/BIS} usados aqui \cite{lee2022,vitaldb,clinicaldata}. O Open Dataset (\texttt{vitaldb.net}) exige CC BY-NC-SA 4.0, \textit{Data Use Agreement} e citação a Lee et al.\ 2022, sem redistribuição. Escala, equipamento, montagem e população diferem do Figshare; por isso, o VitalDB informa sensibilidade à mudança de dataset, sem isolar a causa. O DOSE-I (Zenodo, CC BY 4.0) não foi incorporado à regressão porque usa MOAA/S e estado de consciência, não conversíveis em BIS \cite{dosei}.

A reimplementação aberta de Connor ajuda a discutir transparência algorítmica do índice, mas não torna um regressor treinado contra BIS equivalente ou certificado como monitor \cite{openbis}. A independência entre desenvolvimento e avaliação, a descrição de clusters e a análise de calibração/acordo são componentes centrais de relato e avaliação de modelos; para desfecho contínuo, o tamanho da validação deve ser planejado pela precisão desejada, não pelo número de réplicas bootstrap \cite{tripodai,archer2021}.
...
Cada exemplo contém 640 amostras, isto é, uma janela de 5 s a 128 Hz, com passo de 5 s na construção offline. Para início $t$, o alvo corresponde a $t+\mathrm{offset}$, quantizado por arredondamento ao intervalo BIS do caso (5 s no Figshare e 1 s nos arquivos VitalDB normalizados). A emissão só pode ocorrer após completar a janela, aproximadamente em $t+5$ s, além do processamento; com offset zero, não se trata de previsão de um BIS futuro nem de leitura contemporânea ao fim da janela. O pipeline aplica banda de 0,5--45 Hz, notch de 50 Hz, filtro de quarta ordem, escala de amplitude registrada e rótulo BIS associado ao início da janela (offset padrão 0 s). O estado causal é preservado por caso e entre chunks; uma rota de fase zero existe somente para análise offline explícita. Essas escolhas estão em \artefato{reports/corpus_manifest.json}, \artefato{models/brainsniffer_cnn.json} e \artefato{models/brainsniffer_corpus_fixed.json}; as fontes externas descrevem os corpora e o problema, mas não definem esta configuração \cite{brainsnifferArtefatos,ma2017,nsugbe2022}.

O gate do corpus exige, entre outros critérios internos, pelo menos 90\% de EEG finito, lacuna não finita máxima de 2 s, qualidade global mínima 0,35, pelo menos 80\% de BIS válido e qualidade mínima 0,20 por janela. O caso Figshare 24 permanece em quarentena por escala incompatível. Esses limiares são heurísticas de engenharia ainda não calibradas para risco clínico; no replay, não finitos são rejeitados antes do filtro e baixa qualidade leva à abstenção. A proveniência dos limiares é exclusivamente \artefato{reports/corpus_manifest.json} \cite{brainsnifferArtefatos}.

Esses gates do corpus de desenvolvimento não devem ser confundidos com o tratamento dos 15 casos do benchmark VitalDB histórico. Na construção offline, \texttt{make\_windows} passa o EEG completo do caso ao pré-processador: não finitos são interpolados linearmente entre amostras finitas, com extensão do valor extremo nas bordas; um vetor inteiramente não finito vira zeros. Essa rotina não impõe limite de duração à lacuna e pode usar amostras futuras para imputação, mesmo quando o filtro subsequente é causal. A qualidade é calculada sobre a janela bruta. O construtor descarta alvos BIS não finitos ou fora de $[0,100]$; não interpola o BIS nessa etapa. Antes disso, a normalização em \artefato{src/brainsniffer/data/vitaldb.py}, função \texttt{\_align\_numeric\_to\_eeg}, ordena e remove tempos duplicados e interpola linearmente os pontos BIS finitos numa grade de 1 s relativa ao início do EEG. Interpola apenas entre o primeiro e o último ponto válido; fora desse intervalo mantém NaN. Não há limite de duração das lacunas nem máscara de imputação persistida no NPZ. Aqui, válido para interpolação significa finito, não necessariamente dentro de $[0,100]$. Os 15 arquivos históricos registram EEG não finito; sua avaliação offline difere da política de rejeição de não finitos do replay. Essa imputação pode suavizar lacunas longas e limita a interpretação dos resultados; sua política e as métricas históricas não foram alteradas neste trabalho. A descrição se baseia no código local e nos diagnósticos dos relatórios, não em recomendação dos datasets \cite{brainsnifferArtefatos}.

\subsection{Modelos e treinamento}

A CNN histórica é \texttt{Conv1DDepthEstimator}, com \textit{input shape} $(\mathrm{batch},1,640)$, \textit{Smooth L1 loss} e saída sigmoide (\textit{sigmoid}) escalada para $[0,100]$. A Tabela~\ref{tab:config} explicita a arquitetura compatível em \artefato{src/brainsniffer/models/cnn.py} e os parâmetros registrados nos JSON, sem carregar pesos. Recursos posteriores do treinador e arquiteturas opcionais não são atribuídos retroativamente a esses pesos \cite{brainsnifferArtefatos}.
...
Como controle, o baseline extrai potência por banda, SEF90, entropia espectral, RMS e \textit{line length}, ajustando uma \textit{Random Forest} nas mesmas janelas e partição do ativo. Detalhes em \artefato{reports/spectral_baseline_holdout.json} \cite{brainsnifferArtefatos}.

\subsection{Métricas, incerteza e análises exploratórias}

Foram calculados MAE, RMSE, viés e correlação de Pearson para o rótulo contínuo, além de acurácia e macro-F1 para quatro faixas heurísticas de BIS. As métricas implementadas em \artefato{src/brainsniffer/pipeline/metrics.py} \cite{brainsnifferArtefatos} incluem MAE e RMSE, definidos por
\begin{equation}
\mathrm{MAE}=\frac{1}{n}\sum_{i=1}^{n}|y_i-\hat{y}_i|,
\qquad
\mathrm{RMSE}=\sqrt{\frac{1}{n}\sum_{i=1}^{n}(y_i-\hat{y}_i)^2}.
\end{equation}
O estimando primário é o desempenho pooled por janela aceita, não o desempenho médio de uma pessoa: casos longos pesam mais. O viés é a média de predito menos referência. As faixas são $[0,40)$, $[40,60)$, $[60,80)$ e $[80,100]$. Pearson quantifica associação linear e não acordo; não foram estimados, nesta execução, limites de concordância, coeficiente de concordância ou calibração clínica. As quatro faixas discretas são apenas uma apresentação interna e não constituem classes clínicas independentes \cite{brainsnifferArtefatos,tripodai}.

O bootstrap reamostra casos inteiros com reposição, concatena todas as janelas selecionadas e recalcula a métrica. Assim, preserva melhor a dependência intraclasse que uma reamostragem de janelas, mas o estimando continua ponderado pela duração/número de janelas. Foram usadas $B=1.000$ réplicas, seed 42 e percentis 2,5/97,5 para o intervalo de 95\%. Com apenas cinco casos Figshare, o intervalo é exploratório; 1.000 réplicas não equivalem a 1.000 casos \cite{brainsnifferArtefatos,field2007,archer2021}.
...
Replay significa percorrer uma gravação existente, não adquirir EEG: preenche-se um buffer de 5 s e depois emite-se atualização a cada 1 s, separando valor bruto, EWMA, qualidade, timestamp e origem. O escopo definitivo deste TCC é exclusivamente retrospectivo, sem conexão a EEG físico. A implementação em \artefato{src/brainsniffer/pipeline/realtime.py} distingue o instante de emissão do alvo histórico no início da janela \cite{brainsnifferArtefatos}.

Para visualizar o comportamento temporal, fixou-se antes da inferência o \texttt{case19} do holdout Figshare, por consistência com o caso de demonstração indicado na revisão, não pelo desempenho. Usou-se toda a gravação EEG (4.355 s), sem busca de melhor trecho. O script \artefato{docs/generate_figures.py}, com opção explícita \texttt{--infer-trajectory}, reutiliza \texttt{make\_windows}, configuração do checkpoint ativo e loader restrito, em CPU, dois threads e batch 64, sem treino. A curva bruta (sem EWMA) é posicionada no timestamp do alvo offline, início mais offset 0 s, em passos de 5 s; não representa disponibilidade sem atraso. Arrays, CSV, hashes e métricas ficam em \artefato{tmp/pdfs/trajectory-audit/}; a execução padrão do gerador não faz inferência nem download \cite{brainsnifferArtefatos}.
...
\begin{table}[H]
\centering
\caption{Resultados observados no holdout Figshare comum: cinco casos, 5.523 janelas e seed 42. Fonte: relatórios internos do projeto; métricas agregadas por janela \cite{brainsnifferArtefatos}.}
...
A Figura~\ref{fig:trajectory} mostra queda inicial e elevação final presentes em ambas as curvas, mas a CNN superestima grande parte do patamar intermediário e não reproduz várias oscilações da referência. Nas 850 janelas aceitas desse caso, MAE foi 8,72, RMSE 10,21, viés $+4{,}79$ pontos BIS e Pearson 0,910: associação alta não elimina erro sistemático. É um exemplo ilustrativo, não evidência adicional de generalização. A série BIS contém 906 pontos e estende-se além do EEG; das 871 janelas EEG completas, 21 foram rejeitadas por qualidade, e 35 pontos finais BIS não têm janela EEG correspondente. As 56 posições sem previsão permanecem NaN, sem unir lacunas ou suavizar a referência; não havia EEG não finito neste caso. Fonte: auditoria da inferência em \artefato{tmp/pdfs/trajectory-audit/case19.json} \cite{brainsnifferArtefatos}.

\begin{figure}[tbp]
\centering
\includegraphics[width=\columnwidth]{figures/bis_trajectory.pdf}
\caption{Trajetória offline completa do Figshare \texttt{case19}, preselecionado por consistência da demonstração, não por desempenho. BIS original e CNN ativa bruta (sem EWMA), com erro predito menos referência no painel inferior. Tempo relativo ao início; alvo no início da janela de 5 s, offset 0 s, não no instante de emissão do replay. Lacunas de previsão preservadas; a cauda BIS excede o suporte EEG. Fontes internas: \artefato{data/raw/case19.mat}, checkpoint ativo e auditoria \artefato{tmp/pdfs/trajectory-audit/case19.json}. Exemplo individual, não generalização \cite{brainsnifferArtefatos}.}
...
\begin{table}[H]
\centering
\caption{Comparação retrospectiva nos mesmos benchmarks históricos. ``Visto'' indica fonte presente no desenvolvimento, não uso dos casos de holdout. Pearson mede associação. Fonte: relatórios internos \cite{brainsnifferArtefatos}.}
...
No VitalDB, o MAE do candidato caiu em 14 dos 15 casos pareados; no \texttt{vitaldb\_case16}, aumentou aproximadamente 0,67 ponto BIS. A Figura~\ref{fig:comparison} evidencia que o ganho agregado não elimina heterogeneidade e que casos longos têm maior peso no agregado. Essa comparação, derivada de \artefato{reports/vitaldb_external_validation.json} e \artefato{reports/mixed_vitaldb_external.json}, descreve adaptação após exposição ao domínio VitalDB e não transporte para uma nova fonte \cite{brainsnifferArtefatos}.

\begin{figure}[tbp]
\centering
\includegraphics[width=\columnwidth]{figures/comparison.pdf}
\caption{Agregados por janela no Figshare (5 casos/5.523 janelas) e VitalDB (15/38.730) e MAE pareado por caso, ordenado pela diferença misto menos ativo. Pareamento por \texttt{case\_id} e contagem de janelas. O ativo foi desenvolvido somente no Figshare; o candidato misto já viu outros participantes VitalDB durante o desenvolvimento. Pontos e linhas vêm dos quatro relatórios de holdout citados no texto; 14/15 casos melhoram, mas o caso 16 piora cerca de 0,67. Resultado exploratório, não validação clínica \cite{brainsnifferArtefatos}.}
...
\caption{Métricas observadas (marcadores) e intervalos percentis de 95\% (barras) do bootstrap agrupado por caso ($B=1.000$, seed 42). Casos sorteados contribuem com todas as suas janelas; portanto, a métrica agregada é ponderada por janelas. O $n=5$ do Figshare limita a leitura. Fonte: quatro relatórios internos de holdout \cite{brainsnifferArtefatos}.}
\label{fig:bootstrap}
\end{figure}
...
\caption{Sensibilidade pós-hoc ao alinhamento do rótulo em cinco casos Figshare. Pontos e segmentos preservam a grade e o suporte histórico original: o número de janelas varia de 5.503 a 5.525; não há restrição a suporte comum nem recálculo. A linha vertical indica o padrão 0 s. Os eixos ampliados tornam diferenças pequenas legíveis; a análise sem retreino gera hipótese e não seleciona atraso. Fonte: \artefato{reports/offset_sensitivity.json} \cite{brainsnifferArtefatos}.}
\label{fig:offset}
\end{figure}
...
A degradação do ativo Figshare-only no VitalDB é evidência interna de sensibilidade à mudança de dataset, não prova de qual diferença de equipamento, centro, ganho, montagem, população ou distribuição BIS a causou. A redução de erro do candidato misto é compatível com adaptação após incorporar participantes VitalDB ao desenvolvimento; como o benchmark é da mesma fonte e já havia sido observado, ele deve gerar uma hipótese para uma nova coorte travada, não uma alegação de generalização externa \cite{brainsnifferArtefatos,lee2022,tripodai,imdrfn88}.
